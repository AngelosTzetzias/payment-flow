...
- Use the skills: `backend-feature`, `rn-screen`, `truelayer`. Use the `test-writer` and `code-reviewer` subagents before committing non-trivial work. For multi-step work, track progress with the `todo` skill.
- **Multi-commit cadence (default for a staged slice).** Open the PR from the first commit, then push each subsequent commit as it lands green (don't batch them to the end). After each push, ask the user whether they want to manually test that commit before continuing.
...
- **Scripts and config must be cross-platform.** Devs run on Windows as well as Linux/CI. Don't use Unix-only shell syntax in npm scripts — set env vars via `cross-env` (e.g. `cross-env NODE_OPTIONS=… jest`), not `VAR=… cmd`. Line endings are pinned to LF by `.gitattributes` and Prettier runs `endOfLine: "auto"`, so keep both in place.
- Commands: `pnpm install`, `pnpm db:up`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm -r build`.

## Tech

pnpm workspaces · Node 22 · NestJS 10 + Prisma 7 + Postgres 16 · Next.js (checkout) · Expo/React Native (merchant) · NestJS WebSocket gateway for realtime (no Pusher/Supabase).

> **Backend is ESM + Prisma 7.** The `apps/backend` package is `"type": "module"` (tsconfig `NodeNext`), so relative imports need explicit `.js` extensions. Prisma 7 is Rust-free: the client is generated by the `prisma-client` generator into `src/generated/prisma` (gitignored — run `prisma generate`) and connects via the `@prisma/adapter-pg` driver adapter; the connection URL lives in `prisma.config.ts`, not the schema datasource block.
